...
* 在集群中执行标准的 xref:docs.adoc#java-docs-index[index], xref:docs.adoc#java-docs-get[get],
xref:docs.adoc#java-docs-delete[delete] 和 xref:search.adoc#java-search[search] 操作
* 在运行中的集群下执行管理员任务

在Elasticsearch中获取 `Client` 很简单。
最常用的方法就是创建一个 xref:transport-client[TransportClient] 连接到集群中，并获取client。
...
，然后在连接到该协调节点的应用中轻松地创建一个 xref:transport-client[TransportClient] 。
