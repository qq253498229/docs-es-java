...
如 <<./usage.adoc#java-rest-low-usage-initialization,初始化>> 中所述，`RestClientBuilder` 同时支持提供 `RequestConfigCallback` 和 `HttpClientConfigCallback`，允许 Apache Async Http Client 暴露的任何自定义内容。
