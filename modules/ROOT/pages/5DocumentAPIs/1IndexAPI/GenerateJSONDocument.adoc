...
- 使用第三方库(例如: http://wiki.fasterxml.com/JacksonHome[Jackson])来把你的Bean转换成Json格式
...
唯一需要注意的是你需要通过 https://www.elastic.co/guide/en/elasticsearch/reference/5.6/mapping-date-format.html[Date Format]
对日期进行格式化。
...
你可以使用 http://wiki.fasterxml.com/JacksonHome[Jackson]将你的Bean序列化成JSON。
请将 http://mvnrepository.com/artifact/com.fasterxml.jackson.core/jackson-databind[Jackson Databind]加入你的项目中。
