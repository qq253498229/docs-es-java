...
除了修改文档的 **_source** 字段之外，您还可以使用脚本来修改操作，类似修改 xref:./UpdateAPI.adoc[Update接口]。
...
与 xref:./UpdateAPI.adoc[Update接口] 一样，您可以设置 **ctx.op** 来改变执行的操作。
...
你也可以一次对多个索引和类型执行这些操作，类似于 xref:6SearchAPI/readme.adoc[Search接口]
