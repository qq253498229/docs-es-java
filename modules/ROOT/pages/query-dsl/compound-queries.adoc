...
xref:java-query-dsl-constant-score-query[`constant_score` query]::

包含其它查询的查询，但却以filter context的形式执行该语句。所有匹配到的文档都会分配相同的 "constant" `_score` 。

xref:java-query-dsl-bool-query[`bool` query]::
...
xref:java-query-dsl-dis-max-query[`dis_max` query]::

一个接受多个查询语句的查询，匹配到任何查询子句的文档都会返回。
虽然 `bool` 语句已经将所有匹配结果的得分组合了起来，但 `dis_max` 的得分却是单条语句匹配的最高得分。

xref:java-query-dsl-function-score-query[`function_score` query]::

使用函数修改主查询返回的分数，以考虑流行性、失效性、距离或使用脚本实现的自定义算法等因素。

xref:java-query-dsl-boosting-query[`boosting` query]::

查询条件将会分为正匹配和负匹配。 `positive` （正确）匹配到的文档将会加分，`negative` （负面）匹配得到的文档将会减分。

include::constant-score-query.adoc[]
include::bool-query.adoc[]
include::dis-max-query.adoc[]
include::function-score-query.adoc[]
include::boosting-query.adoc[]
