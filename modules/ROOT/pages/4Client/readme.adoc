...
- 在一个已有的集群中执行标准的 xref:5DocumentAPIs/IndexAPI.adoc[index]，xref:5DocumentAPIs/GetAPI.adoc[get]，xref:5DocumentAPIs/DeleteAPI.adoc[delete] 和 xref:6SearchAPI/readme.adoc[search] 操作。
- 在一个已有的集群中执行管理任务

获取 ES **client** 很简单。最常用的方法就是创建一个连接到集群的 xref:4Client/readme.adoc[TransportClient]，并从中获取client。
...
你可以点击 (https://github.com/elastic/elasticsearch/issues/27205[这里] 把你的应用需要添加的api告诉我们。
任何尚未支持的API仍然可以使用 https://www.elastic.co/guide/en/elasticsearch/client/java-rest/6.2/java-rest-low.html[low level Java REST Client] 通过json的方式来调用。
...
在本地 https://www.elastic.co/guide/en/elasticsearch/reference/6.2/modules-node.html#coordinating-only-node[连接到一个节点] ，
然后在应用中创建一个 xref:_TransportClient[TransportClient]吧。
